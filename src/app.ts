const calculate = document.querySelector(".form") as HTMLFormElement;
const selectDiv = document.querySelector(".select") as HTMLSelectElement;
const select = selectDiv.querySelectorAll("select");
const bandDiv = document.querySelector(".body") as HTMLDivElement;
const bands = bandDiv.querySelectorAll("div");
const color = document.querySelector(".colors") as HTMLDivElement;
const colors = color.querySelectorAll("div");
const response = document.querySelector(".value") as HTMLDivElement;

const representation:{[index:string]:number}= {black:0, brown:1,red:2,orange:3,yellow:4,green:5,
...
function cac (): void{
    ...
    let value: number;
    let tolerance: {[index:number]:number} ={ 1:1,2:2,5:0.5,6:0.25,7:0.1,8:0.05,10:5,11:10}
    const band: number[]= [];
